...
float BaseEffectModule::GetParameterAsMagnitude(int parameter_id) const {
    // Обратное к SetParameterAsMagnitude: возвращает 0..1 с учётом кривой.
    ParameterValueType paramType = GetParameterType(parameter_id);

    if (paramType == ParameterValueType::Float) {
        const float min = (float)GetParameterMin(parameter_id);
        const float max = (float)GetParameterMax(parameter_id);
        const float v = GetParameterAsFloat(parameter_id);
        if (max <= min) {
            return 0.0f;
        }
        float m;
        ...
        case ParameterValueCurve::Log: {
            const float minToUse = std::max(0.01f, min);
            if (v <= 0.0f) {
                return 0.0f;
            }
            m = (std::log10(v) - std::log10(minToUse)) / (std::log10(max) - std::log10(minToUse));
            break;
        }
        case ParameterValueCurve::InverseLog: {
            ...
            m = 1.0f - (std::log10(v) - std::log10(minToUse)) / (std::log10(max) - std::log10(minToUse));
            break;
        }
        ...
        return GetParameterAsBool(parameter_id) ? 1.0f : 0.0f;
    } else if (paramType == ParameterValueType::Binned) {
        const int binCount = GetParameterBinCount(parameter_id);
        if (binCount <= 1) {
            return 0.0f;
        }
        return (float)(GetParameterAsBinnedValue(parameter_id) - 1) / (float)(binCount - 1);
    }
    return 0.0f;
}
...
void BaseEffectModule::SetParameterAsBinnedValue(int parameter_id, int value) {
    ...
    // Make sure that this is a binned type value.
    if (binCount == -1) {
        return;
    }

    // Clamp into range (instead of rejecting): пресет с бином вне набора даёт
    // предсказуемую крайнюю модель/кабинет, а не «застрявшее» прошлое значение.
    if (value < 1) value = 1;
    if (value > binCount) value = binCount;
